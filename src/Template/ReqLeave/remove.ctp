...
<html lang="en">
  ...
  <body>
    ...
    <div class="bodytransition">
      <div class="bodypart">
        ...
        <div class="col"><h2>Unapproved Requested Leave</h2></div>
        <?php 

        ?>
        <button type="button" class="btn outlineblue mr-2"><a href="<?php echo Router::url(['controller'=>'Dashboard','action'=>'index']) ?>">Cancel</a></button>
  ...
  <?php if($reqLeave->approval_states=="remove"||strtolower($reqLeave->approval_states)=="done"){
    continue;
  }?>
    <tr>
    <?php  $test=0?>
        ...
        $y=0;
                ...
                $emp=mysqli_query($conn,"Select * from req_leave where empId=$reqLeave->empId && leave_year=$reqLeave->leave_year");
                {
                  while($r1=mysqli_fetch_assoc($emp)){
                    ...
                    &&($reqLeave->starting_date->format("Y-m-d") >= $r1['starting_date'])){
